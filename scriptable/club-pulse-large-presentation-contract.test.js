...
check('Large routes without altering the existing Medium renderer',patch.includes("if(family==='large')return buildLarge(d,imgs)")&&patch.includes('return CP_LARGE_BASE_BUILD_MEDIUM(d,imgs)'));
check('Large presentation adds no new API calls',!patch.includes('await api(')&&!patch.includes('await liveApi('));

check('Launcher pins Large Presentation v1 to an immutable commit',/const LARGE_PRESENTATION_PATCH='https:\/\/raw\.githubusercontent\.com\/48wr9f4wgp-lab\/club-pulse\/[0-9a-f]{40}\/scriptable\/club-pulse-large-presentation-patch\.js';/.test(launcher));
check('Launcher uses Large Presentation v1 local cache',launcher.includes('ClubPulseLargePresentationPatch_v1.js')&&launcher.includes("'large-presentation1'"));
check('Large patch is injected after the existing patch stack',launcher.includes("const runtime=b.replace(M,lrg+'\\n'+M)"));
check('In-app Large presentation uses native presentLarge when family is Large',launcher.includes("family==='large'?await widget.presentLarge():await widget.presentMedium()"));
