...
check('Large keeps a fixed recent-five form section',patch.includes("text(summary,'直近5試合'")&&patch.includes('values.length<5'));
...
check('Large enriches next match from already-fetched standings',patch.includes('const CP_LARGE_BASE_MAP_DATA=mapData')&&patch.includes('cpLargeStandingRow(sj,mapped.opponentId)')&&patch.includes('opponentRank:row?.position??null'));
check('Large derives round and kickoff countdown context',patch.includes('function cpLargeRoundLabel(raw)')&&patch.includes('function cpLargeCountdown(m)')&&patch.includes("items.push(`相手 ${m.opponentRank}位`)")&&patch.includes('items.push(countdown)'));
check('Large recent form includes record and goals summary',patch.includes('function cpLargeFormSummary(rows)')&&patch.includes('function cpLargeFormRecord(d)')&&patch.includes('`${s.goalsFor}得点 ${s.goalsAgainst}失点`'));
check('Previous-result context includes round label',patch.includes("const pieces=[m.kickoff,m.roundLabel,m.venue||'会場未定']"));
...
check('Launcher pins Large Presentation v4 to an immutable commit',/const LARGE_PRESENTATION_PATCH='https:\/\/raw\.githubusercontent\.com\/48wr9f4wgp-lab\/club-pulse\/[0-9a-f]{40}\/scriptable\/club-pulse-large-presentation-patch\.js';/.test(launcher));
check('Launcher uses Large Presentation v4 local cache',launcher.includes('ClubPulseLargePresentationPatch_v4.js')&&launcher.includes("'large-presentation4'"));
